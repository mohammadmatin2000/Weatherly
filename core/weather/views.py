...
class WeatherAPIView(APIView):
    def get(self, request, format=None):
        ...
        for item in data.get('list', []):  # پیمایش لیست پیش‌بینی‌ها
            date_str, time_str = item['dt_txt'].split(' ')  # جدا کردن تاریخ و ساعت
            if date_str not in seen_dates and time_str == "12:00:00":
                ...
                days.append({
                    ...
                    "weather_desc": item['weather'][0]['description'],  # توضیح آب و هوا
                    "icon": item['weather'][0]['icon']  # آیکون وضعیت آب و هوا
                })
                seen_dates.add(date_str)  # اضافه کردن تاریخ به set برای جلوگیری از تکرار
